...
import {
  ProvenanceGroupReviewError,
  reviewProvenanceGroupProposal,
  type ProvenanceGroupReviewDecision,
} from "../../../../lib/server/provenance-group-review";

export const prerender = false;
const decisions = new Set<ProvenanceGroupReviewDecision>(["accept", "reject"]);
...
export const PATCH: APIRoute = async ({ request, locals }) => {
  ...
  if (typeof decision !== "string" || !decisions.has(decision as ProvenanceGroupReviewDecision)) return Response.json({ error: "decision must be accept or reject." }, { status: 400 });
  ...
  try {
    const result = await reviewProvenanceGroupProposal(env.DB as D1Database, {
      proposalId,
      decision: decision as ProvenanceGroupReviewDecision,
    ...
    if (error instanceof ProvenanceGroupReviewError) return Response.json({ error: error.message, code: error.code }, { status: error.status });
    console.error("provenance group review failed", error);
    return Response.json({ error: "Provenance group review could not be saved." }, { status: 500 });
  }
};
