...
import {
  ClaimMatchReviewError,
  reviewClaimMatchCandidate,
  type ClaimMatchDecision,
} from "../../../../lib/server/claim-match-review";

export const prerender = false;

const decisions = new Set<ClaimMatchDecision>(["merge_existing", "create_new", "reject"]);
...
export const PATCH: APIRoute = async ({ request, locals }) => {
  ...
  if (!/^[A-Za-z0-9_-]{4,240}$/.test(candidateId)) return Response.json({ error: "Invalid candidateId." }, { status: 400 });
  if (typeof decision !== "string" || !decisions.has(decision as ClaimMatchDecision)) {
    return Response.json({ error: "decision must be merge_existing, create_new, or reject." }, { status: 400 });
  }
  ...
  try {
    const result = await reviewClaimMatchCandidate(env.DB as D1Database, {
      candidateId,
      decision: decision as ClaimMatchDecision,
    ...
    if (error instanceof ClaimMatchReviewError) return Response.json({ error: error.message, code: error.code }, { status: error.status });
    console.error("claim-match review failed", error);
    return Response.json({ error: "Claim-match review could not be saved." }, { status: 500 });
  }
};
