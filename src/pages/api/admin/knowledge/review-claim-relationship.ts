...
import {
  ClaimRelationshipReviewError,
  reviewClaimRelationshipProposal,
  type ClaimRelationshipReviewDecision,
} from "../../../../lib/server/claim-relationship-review";

export const prerender = false;
const decisions = new Set<ClaimRelationshipReviewDecision>(["accept", "reject"]);
...
export const PATCH: APIRoute = async ({ request, locals }) => {
  ...
  if (typeof decision !== "string" || !decisions.has(decision as ClaimRelationshipReviewDecision)) return Response.json({ error: "decision must be accept or reject." }, { status: 400 });
  ...
  try {
    const result = await reviewClaimRelationshipProposal(env.DB as D1Database, {
      proposalId, decision: decision as ClaimRelationshipReviewDecision,
    ...
    if (error instanceof ClaimRelationshipReviewError) return Response.json({ error: error.message, code: error.code }, { status: error.status });
    console.error("claim relationship review failed", error);
    return Response.json({ error: "Claim relationship review could not be saved." }, { status: 500 });
  }
};
