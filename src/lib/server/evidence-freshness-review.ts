...
function validateInput(input: RequestFreshnessReviewInput): void {
  if (!text(input.claimAssertionId)) throw new EvidenceFreshnessReviewError("claimAssertionId is required.", "invalid_assertion_id");
  if (!text(input.actor)) throw new EvidenceFreshnessReviewError("actor is required.", "invalid_actor");
  if (!text(input.reason) || input.reason.length > 2000) throw new EvidenceFreshnessReviewError("A bounded review reason is required.", "invalid_reason");
  if (!text(input.idempotencyKey) || input.idempotencyKey.length > 256) throw new EvidenceFreshnessReviewError("A bounded idempotency key is required.", "invalid_idempotency_key");
...
export async function requestFreshnessReview(
  ...
  if (existing) {
    return {
      ...
      claimAssertionId: existing.claim_assertion_id,
      priorState: existing.prior_state,
      proposedState: existing.proposed_state,
      inserted: false,
      replay: true,
    };
  }
  ...
  await db.prepare(`
    INSERT INTO evidence_freshness_reviews
      (id, claim_assertion_id, prior_state, proposed_state,
       source_document_version_id, reason, requested_by, idempotency_key)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    reviewId, row.id, row.freshness_state, input.proposedState,
    input.sourceDocumentVersionId ?? row.source_document_version_id ?? null,
    input.reason.trim(), input.actor.trim(), input.idempotencyKey,
  ).run();
  return {
    reviewId,
    state: "pending",
    claimAssertionId: row.id,
    priorState: row.freshness_state,
...
export async function approveFreshnessReview(
  ...
  if (!text(reviewId) || !text(actor)) throw new EvidenceFreshnessReviewError("reviewId and actor are required.", "invalid_approval");
  if (reviewNote.length > 2000) throw new EvidenceFreshnessReviewError("The review note is too large.", "invalid_review_note");
