...
async function validationAndUnsupportedTests(): Promise<void> {
  ...
  try {
    ...
    const pdf = await upload(database, rawStore, "%PDF-1.7\nopaque bytes\n%%EOF\n", "document.pdf", "application/pdf");
    assert.equal(pdf.state, "extraction_pending");
    ...
    assert.equal(pdfVersion?.extraction_state, "pending");
    assert.equal(pdfVersion?.storage_state, "private_stored");
    assert.equal(pdfVersion?.state_reason, "pdf_extraction_pending");
    assert.equal(rawStore.objects.size, 1, "valid PDFs write only the private original artifact");
