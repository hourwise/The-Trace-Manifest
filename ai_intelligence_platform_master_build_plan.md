...
| LAUNCH-05R | Repair the Access, D1 binding and Pages-to-Worker control plane only after explicit operator approval. | **Partially completed:** the shared HMAC secret is present on Pages production and the ingestion Worker; two manual Access path applications share a narrow policy; their team domain and two-AUD allowlist are encrypted Pages secrets. A read-only production D1 inspection found a partial pre-stabilisation schema, and the Pages environment binding has been corrected in source. A non-production Pages deployment is active behind Cloudflare Access, and an isolated preview Worker is deployed with preview-only D1/R2 bindings, AI off, and no cron schedule. Its Pages variables, distinct HMAC pairing, authenticated checks, and audit/replay tests remain pending. `docs/audit/launch-05r-control-plane-repair-evidence.md` and `docs/audit/launch-05r-preview-deployment-evidence.md` record redacted proof. Production migration approval and smoke tests remain blocking. Do not remove the legacy secret first. |
...
| DESK-02 | Verify TRACE Desk route boundaries and manual candidate intake. | **In progress:** live anonymous requests are held by Cloudflare Access and the regression suite verifies unsigned rejection, reader denial, and publisher-only creation of an attributable `new` candidate without fetching or publishing. Repeat the authenticated browser check against a migrated non-production Pages environment (or after an explicitly approved production migration); the production D1 remains unchanged. See `docs/audit/desk-02-route-boundary-evidence.md`. |
...
| LAUNCH-05R repair | Shared HMAC secret completed and verified on Pages production and the ingestion Worker; two Access path applications and encrypted Pages team-domain/two-AUD configuration are now evidenced; the source now defines separate preview and production D1 bindings. A read-only inspection found production is pre-stabilisation. Legacy secret intentionally retained. Deployed binding evidence, production backup/migration approval, preview controls and smoke tests are incomplete. | Deploy the corrected binding configuration, then separately approve and run the production backup plus stabilisation-only migration before LAUNCH-06. |
