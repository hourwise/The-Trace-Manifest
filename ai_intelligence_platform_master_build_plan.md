...
| LAUNCH-05R | Repair the Access, D1 binding and Pages-to-Worker control plane only after explicit operator approval. | **Partially completed:** the shared HMAC secret is now present on Pages production and the ingestion Worker; `docs/audit/launch-05r-control-plane-repair-evidence.md` records the redacted proof. Access permission, Access application/AUD, deployed Pages binding evidence, preview controls and smoke tests remain blocking. Do not remove the legacy secret first. |
...
| LAUNCH-05R repair | Shared HMAC secret completed and verified on Pages production and the ingestion Worker; legacy secret intentionally retained. Access application/AUD, deployed Pages/preview binding evidence and smoke tests are incomplete. | Obtain `Access: Apps and Policies Write` authority or configure Access in the dashboard, then capture the redacted evidence packet before LAUNCH-06. |
