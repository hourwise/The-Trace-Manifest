...
| LAUNCH-05R repair | Shared HMAC secret completed and verified on Pages production and the ingestion Worker; two Access path applications and encrypted Pages team-domain/two-AUD configuration are now evidenced; legacy secret intentionally retained. Role allowlists, OTP-only login restriction, deployed Pages/preview binding evidence and smoke tests are incomplete. | Set the Pages role allowlists, restrict the dashboard applications to One-time PIN, confirm bindings and fresh deployment, then capture the redacted evidence packet before LAUNCH-06. |
