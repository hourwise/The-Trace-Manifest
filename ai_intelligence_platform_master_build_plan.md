...
| LAUNCH-05R | Repair the Access, D1 binding and Pages-to-Worker control plane only after explicit operator approval. | **Production D1 migration complete; deployment pending:** a backup and Time Travel recovery point were captured, then the legacy claims/evidence and missing catalogue forward repairs, stabilisation/security migration, and TRACE Desk migration were applied and verified with `quick_check`, foreign-key validation, preserved feed/cluster counts, seven sections, and exactly nine discovery sources. No AI request, audit action, candidate, publication, or ingestion was initiated. See `docs/audit/launch-05r-production-d1-migration-evidence.md`. Preview Access, Pages role mapping, isolated Preview D1 binding, distinct HMAC pairing, safe publisher-only Desk submission, and redacted audit outcome remain verified; Preview AI remains off and the Preview Worker has no cron schedule. The remaining scope is merge/deploy from `main` and no-write production smoke checks. Do not remove the legacy secret first. |
...
| LAUNCH-05R repair | Production D1 compatibility, stabilisation/security, and TRACE Desk migrations are complete and verified; backup and recovery evidence is recorded in `docs/audit/launch-05r-production-d1-migration-evidence.md`. Legacy secret intentionally retained. | Merge the reviewed release branch to `main`, confirm the Pages production deployment, dry-run then deploy the production Worker, and perform no-write smoke checks before LAUNCH-06. |
