...
### KC-03 — Safe content absorption (complete; KC-03A–H complete)
...
- [x] **KC-03F:** Add publisher-only document upload for supported text/HTML/Markdown inputs, with type, size, retention, and untrusted-content controls. The upload reuses the canonical source/version, private artifact, deterministic extraction/chunk, reconciliation, audit, and review-gated candidate paths; it cannot publish or establish accepted evidence.
- [x] **KC-03G:** Add explicit retrieval, capture, extraction, and storage states for unavailable, paywalled, robots/policy restricted, unsupported, extraction failed, and metadata-only sources, with durable diagnostics, retryability, and fail-closed downstream eligibility.
- [x] **KC-03H:** PDF v1 stores the permitted original privately in R2 with metadata and hash, and records `extraction_pending` or `metadata_only`; it does not add a parser to the main ingestion Worker.
...
Exit: one admitted feed article, one editor-supplied ordinary text document, and one permitted PDF can be captured, versioned, retrieved privately, and audited without entering public evidence automatically; PDF bytes remain opaque, with no parser, chunks, claims, embeddings, citations, or publication. KC-03A evidence: [`kc-03a-shared-source-retrieval-evidence.md`](audit/kc-03a-shared-source-retrieval-evidence.md). KC-03B evidence: [`kc-03b-html-extraction-evidence.md`](audit/kc-03b-html-extraction-evidence.md). KC-03C evidence: [`kc-03c-private-source-capture-evidence.md`](audit/kc-03c-private-source-capture-evidence.md). KC-03D evidence: [`kc-03d-feed-capture-queue-evidence.md`](audit/kc-03d-feed-capture-queue-evidence.md). KC-03E evidence: [`kc-03e-capture-consumer-manual-url-evidence.md`](audit/kc-03e-capture-consumer-manual-url-evidence.md). KC-03F/G evidence: [`kc-03f-03g-document-upload-source-states-evidence.md`](audit/kc-03f-03g-document-upload-source-states-evidence.md). KC-03H evidence: [`kc-03h-pdf-private-capture-evidence.md`](audit/kc-03h-pdf-private-capture-evidence.md).
...
- [ ] **KC-11C:** Backfill admitted source documents and immutable versions in bounded, Preview-only, publisher-approved batches. The corrective implementation adds immutable KC-11A snapshots with append-only authority generations, approval invalidation, exact execution leases/idempotency, stale-lease recovery, retry exhaustion, strict byte/time ceilings, and field-level ledger immutability. It remains gated until migration 0058 and the corrective Preview deployment are verified, a human-approved Preview smoke batch succeeds, and remote ledger outcomes are checked. See [`docs/audit/kc-11c-bounded-source-backfill-evidence.md`](audit/kc-11c-bounded-source-backfill-evidence.md).
