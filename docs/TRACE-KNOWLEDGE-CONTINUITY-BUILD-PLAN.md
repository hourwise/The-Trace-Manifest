...
- [x] **KC-11C:** Backfill admitted source documents and immutable versions in bounded, Preview-only, publisher-approved batches. Plans require an explicit KC-11A inventory selection, deterministic hash, and bounded ceilings; execution reuses the existing normalization, SSRF-safe retrieval, admission, copyright storage, duplicate/version, and idempotent capture boundaries. The append-only batch/item ledger records exclusions, metadata-only outcomes, unavailable sources, retryable failures, and partial settlement. No production target, AI, Vectorize, scoring, claims, or public presentation is enabled. See [`docs/audit/kc-11c-bounded-source-backfill-evidence.md`](audit/kc-11c-bounded-source-backfill-evidence.md).
