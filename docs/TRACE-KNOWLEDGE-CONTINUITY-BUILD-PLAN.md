...
- [ ] **KC-11C:** Backfill admitted source documents and immutable versions in bounded, Preview-only, publisher-approved batches. The corrective implementation adds authoritative immutable KC-11A snapshots, exact execution leases/idempotency, retry exhaustion, strict byte/time ceilings, and deletion/state-transition protection. It remains gated until migration 0057 is applied, a human-approved Preview smoke batch succeeds, and remote ledger outcomes are verified. See [`docs/audit/kc-11c-bounded-source-backfill-evidence.md`](audit/kc-11c-bounded-source-backfill-evidence.md).
